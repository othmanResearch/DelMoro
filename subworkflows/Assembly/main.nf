...
include {alignReadsToRef;alignReadsToRefBWAMEM2; assignReadGroup; markDuplicates; IndexBam; Extractregion; GenerateStat; IndexBam as IndexRegion} from '../../modules/4_Assembly.nf' 
...
workflow ALIGN_TO_REF_GENOME {
  ...
  if ( params.aligner == null ) {
 	...
 	if ( params.exec != null && 
 	     ...
 	     params.region == null && 
 	     params.bedtarget == null ) { DelMoroAssemblyOutput()
		...
		// Case: CHECK COVERAGE IN TARGETD REGION FROM BED FILE 
			...
			// Case: Region specified Extract BAM REGION FILE
				    ...
				    params.tobealigned != null && 
				    params.generate == null && 
				    (params.region ==~ /^[a-zA-Z0-9]+:\d+-\d+$/) ) {  DelMoroAssemblyOutput()
